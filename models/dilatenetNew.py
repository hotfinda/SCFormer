...
from models.attentions import Dilation3d
...
class LocalAttention(nn.Module):
    ...
    def __init__(self, dim_in, dim_out=None, *,
        ...
        super().__init__()
        self.attn = Dilation3d(dim_in, dim_out,
                                heads=heads, dim_head=dim_head, dropout=dropout, k=k)
        self.window_size = window_size

        self.rel_index = self.rel_distance(window_size-2) + (window_size-2) - 1
        self.pos_embedding = nn.Parameter(torch.randn(2 * (window_size-2) - 1, 2 * (window_size-2) - 1) * 0.02)

    def forward(self, x, mask=None):
        b, c, h, w = x.shape
        p = self.window_size
        p_change = self.window_size-2
        if h//p ==0 :
            p = p//2
        n1 = h // p
        n2 = w // p
 
        mask = torch.zeros(p_change ** 2, p_change ** 2, device=x.device) if mask is None else mask   #p**2需要改为XX，因为实际上的qkv尺寸变小了。按照公式，p*p的图形，经过kernel=2,dilation=2，尺寸变为p-2
...
class AlterNet(nn.Module):
    ...
    def _make_layer(block1, block2, in_channels, out_channels, num_block1, num_block2, stride, heads, sds, **block_kwargs):
        ...
        if num_block2==2:
            alt_seq = [False] * (num_block1 - num_block2 * 2) + [True, True, True,True,True] * (num_block2-1)
        else:
            alt_seq = [False] * (num_block1 - num_block2 * 2) + [True, True, True] * num_block2
...
def dnn_50(num_classes=1000, stem=True, name="alternet_50", **block_kwargs):
    return AlterNet(preresnet_dnn.Bottleneck, AttentionBlockB, stem=partial(StemB, pool=stem),
                    num_blocks=(3, 3, 5, 3), num_blocks2=(0, 1, 2, 1), heads=(3, 6, 12, 24),
                    num_classes=num_classes, name=name, **block_kwargs)
