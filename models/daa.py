...
class LocalAttentionD(nn.Module):

    def __init__(self, dim_in, dim_out=None, *,
                 window_size=8, k=1,
                 heads=8, dim_head=32, dropout=0.0):
        super().__init__()
        self.attn = Attention9d(dim_in, dim_out,
                                heads=heads, dim_head=dim_head, dropout=dropout, k=k)
        self.window_size = window_size
...
class AttentionBlockBD(nn.Module):
    # Attention block with pre-activation.
    # We use this block by default.
    expansion = 4
    ...
    def __init__(self, dim_in, dim_out=None, *,
        ...
        super().__init__()
        dim_out = dim_in if dim_out is None else dim_out
        attn = partial(LocalAttentionD, window_size=window_size, k=k)
        width = dim_in // self.expansion
...
class AlterNet(nn.Module):
    ...
    def __init__(self, block1, block2, block3, *,
        ...
        self.layer1 = self._make_layer(block1, block2, block3, 64, 64,
                                       num_blocks[0], num_blocks2[0], stride=1, heads=heads[0], sds=sds[0], **block_kwargs)
        self.layer2 = self._make_layer(block1, block2, block3, 64 * block2.expansion, 128,
                                       num_blocks[1], num_blocks2[1], stride=2, heads=heads[1], sds=sds[1], **block_kwargs)
        self.layer3 = self._make_layer(block1, block2, block3, 128 * block2.expansion, 256,
                                       num_blocks[2], num_blocks2[2], stride=2, heads=heads[2], sds=sds[2], **block_kwargs)
        self.layer4 = self._make_layer(block1, block2, block3, 256 * block2.expansion, 512,
